...
uint8_t read_fat32_BPB_header(FAT_All_BPB_Head *fat_head , uint64_t base_addr, FILE *fd){
    ...
    uint32_t non_data_sectors = fat_head->RsvdSecCnt + fat_head->NumFATs * fat_head->FAT_size_in_sectors;
    // calc data_addr
    fat_head->data_addr = base_addr + fat_head->sector_size * (non_data_sectors);
...
/*
uint8_t get_next_cluster( uint32_t cluster,  uint32_t *next_cluster ,FAT_All_BPB_Head *fat_head ,uint8_t FAT_Num ){
    if(next_cluster == NULL) return 1;
    if(fat_head == NULL) return 1;
    if(FAT_Num > fat_head->NumFATs)
}*/